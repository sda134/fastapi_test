...
namespace Mtec.Internal.Mitsubishi.MXDebugger
{
    public partial class MainDialog : Form
    {
        ...
        private void setDegree(int degree)
        {
            ...
            if (this._mx.IsOpen && this._semaphore.CurrentCount > 0)
            {
                ...
                Task task = Task.Run(() =>
                {
                    ...
                    for (int x = 0; x < 9; x++)
                    {
                        devices.Add(new UtilityLibrary.Mitsubishi.MXComponent.DeviceFieldFormat
                        {
            ...
            if (this._mx.IsOpen)
            {
                var target =
                    (from table in Config.Instance.FieldTables
                     ...
                     select new Mtec.UtilityLibrary.Mitsubishi.MXComponent.DeviceFieldFormat
                     {
                ...
                if (this.radioButton_encorder_trigger.Checked && Config.Instance.Encorder != null)
                    target.AddRange(Config.Instance.Encorder.Trigger.Select(x => new Mtec.UtilityLibrary.Mitsubishi.MXComponent.DeviceFieldFormat
                    {
                ...
                var deviceList = new List<Mtec.UtilityLibrary.Mitsubishi.MXComponent.DeviceFieldFormat>();
                target.ForEach(x =>
                {
        ...
        private void rebuidControlTab()
        {
            ...
            {
                Width = this.tabControl_fields.Width - this.tabControl_fields.Margin.Horizontal - 30,
        ...
        private void refreshCotrolTab()
        {
            ...
            if (this._mx.IsOpen && _bindingLoggerFields.Count > 0)
            {
                ...
                var deviceList = _bindingLoggerFields.Select(x => new Mtec.UtilityLibrary.Mitsubishi.MXComponent.DeviceFieldFormat
                {
                    DeviceName = x.DeviceName,
                    DeviceFormatType = x.DeviceFormatType,
                }).ToList();

                this._mx.ReadData(ref deviceList);
            ...
            if (!this._mx.IsOpen)
                ...
                switch (e.Value)
                {
                    ...
                    case short shVal:
                        ...
                        this._mx.SetDevice(e.DeviceName, BitConverter.GetBytes(shVal));
                        break;
        ...
        private void tabControl_fields_TabIndexChanged(object sender, EventArgs e)
        {
            if (this.tabControl.SelectedIndex == -1) return;

            if (this.tabControl.TabPages[this.tabControl.SelectedIndex] == this.tabPage_fields_control)
            {
                this.rebuidControlTab();
            }
            /*
