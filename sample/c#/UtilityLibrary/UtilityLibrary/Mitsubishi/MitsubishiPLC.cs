...
namespace Mtec.UtilityLibrary.Mitsubishi
{
   ...
   // [System.Diagnostics.DebuggerDisplay("DeviceName = {DeviceName}")]
    [System.Serializable]
...
{
    // MC プロトコルからのコピー
    // 19.05.03 16進数対応を修正したので、MCプロトコルのものよりこちらが新しい
    // 19.07.15 MCプロトコルのものと統合した


    /// <summary>
    /// 三菱のシーケンサのデバイス名の並び替えなどに使えるクラスです。
    /// 対応CPU, F, L, Q, iQ-L
    /// </summary>
    public class FxDevice : IEquatable<FxDevice>, IComparable<FxDevice>
    {
        ...
        public int DeviceDataBitLentgh
        {
            get
            {
                ...
                {
                    // ビット系 1bit
                    case FxDeviceType.SpecialRelay:

                    case FxDeviceType.InputSignal:
                    case FxDeviceType.OutputSignal:
                    case FxDeviceType.InnerRelay:
                    case FxDeviceType.LatchRelay:
                    case FxDeviceType.Annunciator:
                    case FxDeviceType.EdgeRelay:
                    case FxDeviceType.LinqRelay:
                    case FxDeviceType.Timer_Contact:
                    case FxDeviceType.Timer_Coil:
                    case FxDeviceType.LongTimer_Contact:
                    case FxDeviceType.LongTimer_Coil:
                    case FxDeviceType.IntegratedTimerContact:
                    case FxDeviceType.IntegratedTimer_Coil:
                    case FxDeviceType.LongIntegratedTimer_Contact:
                    case FxDeviceType.LongIntegratedTimer_Coil:
                    case FxDeviceType.Counter_Contact:
                    case FxDeviceType.Counter_Coil:
                    case FxDeviceType.LongCounter_Contact:
                    case FxDeviceType.LongCounter_Coil:
                    ...
                    // ワード系 16bit = 2byte
                    case FxDeviceType.SpecialRegister:
                    case FxDeviceType.DataRegister:
                    case FxDeviceType.LinqRegister:
                    case FxDeviceType.Timer_Value:
                    case FxDeviceType.IntegratedTimer_Value:
                    case FxDeviceType.Counter_Value:
                    case FxDeviceType.LinqSpecialRegister:
                    case FxDeviceType.IndexRegister:
                    case FxDeviceType.FileRegister:
                    case FxDeviceType.FileRegister_ZR:
                    case FxDeviceType.RefreshDataRegister:
                        return 16;

                    // ダブルワード系 16bit = 2byte
                    case FxDeviceType.LongTimer_Value:
                    case FxDeviceType.LongIntegratedTimer_Value:
                    case FxDeviceType.LongCounter_Value:
                    case FxDeviceType.LongIndexRegister:
                        return 32;


                    // よくわからない
                    case FxDeviceType.UnitBuffer:
                    case FxDeviceType.UnitBufferHG:
                    default:
                        return 0;
                }
            }
        ...
        public override string ToString()
        {
            ...
            switch (this.DeviceType)
            {
                ...
                // 以下のデバイスは16進数対応ができるようにする
                // FX5, FX3 では8進数   iQ-L,Lなどでは16進数
                case FxDeviceType.InputSignal:
                case FxDeviceType.OutputSignal:
                case FxDeviceType.LinqRelay:
                case FxDeviceType.LinqRegister:
                ...
                case FxDeviceType.DirectAccessOutput:
                case FxDeviceType.FileRegister_ZR:
                    deviceNumStr = Convert.ToString(this.DeviceNumber, 16);
                    break;

                // 10進数
                default:
                    deviceNumStr = this.DeviceNumber.ToString();
                    break;

                    #endregion
            }
            return string.Format("{0}{1}", this.DeviceType.ToDeviceLetter(), deviceNumStr);
        }
        ...
        public static bool TryParse(string s, out FxDevice result)
        {
            var ret = default(FxDevice);
        ...
        public static implicit operator FxDevice(string arg)
        {
            FxDevice instance = null;
            ...
            // ひらがなとカタカナを区別しない IndexOf(s1, "ウエ", System.Globalization.CompareOptions.IgnoreKanaType))
            // 半角と全角を区別しない IndexOf(s1, "ｳｴ", System.Globalization.CompareOptions.IgnoreWidth));
            ...
            foreach (var dev in devList)
            {
                if (arg.StartsWith(dev.devString))
                {
                    ...
                    if (isNumeric)
                    {
                        instance = new FxDevice();
                        instance.DeviceType = dev.devType;
                        instance.DeviceNumber = devNum;
            ...
            // 例外をスロー
            throw (new FormatException("書式が異なります"));
